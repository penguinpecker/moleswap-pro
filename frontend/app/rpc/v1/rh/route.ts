/**
 * /rpc/v1/rh — Robinhood Chain's RPC proxy, the twin of /rpc/v1/arc.
 *
 * WHY IT EXISTS, added 2026-08-24. The Robinhood endpoint used to reach the browser as
 * `NEXT_PUBLIC_RH_RPC_URL`, and NEXT_PUBLIC_ means "inline this into the client bundle at build
 * time" — so a keyed Alchemy URL was sitting in three shipped JavaScript chunks on the live site,
 * readable by anyone who opened the page. Arc has been proxied since it launched, for precisely the
 * reason written at the top of lib/rpc/upstreams.ts. Robinhood never got the same treatment.
 *
 * With this in place the browser is told a URL of OURS, so a leaked or exhausted upstream can be
 * rotated without a single user touching their wallet settings.
 */
/**
 * /rpc/v1/rh — MoleSwap's public JSON-RPC endpoint for Robinhood Chain mainnet (5042).
 *
 * Paste https://www.moleswap.com/rpc/v1/rh into MetaMask (or any wallet, or viem/ethers) as
 * the Robinhood Chain RPC URL. POST speaks JSON-RPC 2.0; GET returns a machine-readable descriptor
...
import { rhUpstreams } from "@/lib/rpc/upstreams";
 ...
 * itself) gets everything needed to add Robinhood Chain, including the decimals trap spelled out.
 */
  ...
  return NextResponse.json(
    {
      ...
      notes: {
        gasToken:
          "Robinhood Chain charges gas in USDC. The native balance is 18-decimal; the ERC-20 facade over the same balance reports 6 decimals.",
...
export async function POST(req: NextRequest) {
  ...
  if (!limit.allowed) {
    return new NextResponse(
      JSON.stringify({
        ...
        error: { code: -32005, message: "Rate limit exceeded on the public MoleSwap Robinhood Chain RPC" },
      }),
      {
        status: 429,
        headers: {
  ...
  const result = await proxyJsonRpc({ rawBody, upstreams: rhUpstreams() });
