  ...
  quote(params: {
    ...
    if (this.states.length > 0) {
      try {
        ...
        // Only the honest no-route answer becomes null; anything else is a defect on our side and is
        // rethrown, so a live quoter crash never renders as "no route" (learnings.txt 2026-08-14 #2).
        if (err instanceof NoRouteError) tickQ = null;
        else throw new QuoteFailedError(err);
      }
    }
